...
public class ModuleManager {
    ...
    private static void executeCommand(ChatMessage message, CommandData data, Matcher m){
        ...
        try{
            ...
            Keyboard.type("Failed... (" + ExceptionUtils.getStackTrace(e) + ")");
        }

        try{
            methodAccessor.invoke(null, a.toArray());
        }catch(Exception e){
            Keyboard.type("Failed... (" + Utils.upload(ExceptionUtils.getStackTrace(e)) + ")");
        }
    ...
    public static void parseText(ChatMessage message){
        ...
        if(command.startsWith(SkypeNet.COMMAND_PREFIX)){
        ...
        for(Map.Entry<String, CommandData> s : allCommands.entrySet()){
            ...
            if(s.getValue().getCommand().command()){
                match = SkypeNet.COMMAND_PREFIX + match;
            }
        ...
        if(allCommands.containsKey(commandSplit[0])){
            ...
            if(c.command()){
                ...
                correct = SkypeNet.COMMAND_PREFIX + correct;
            }

            Keyboard.type("Incorrect syntax: " + correct);

            return;
        }

        if(originalCommand.startsWith(SkypeNet.COMMAND_PREFIX)){
            Keyboard.type("Command '" + commandSplit[0] + "' not found!");
        }
    }
